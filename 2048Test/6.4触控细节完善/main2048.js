...
function generateOneNumber(){
    ...
    if(nospace(board)){
        ...
        //因为randx是坐标,所以必须将值类型parseInt转换为整形int
    ...
    switch(event.keyCode){
        case 37: //按下left键
            if(moveLeft()){ //判断当前的棋盘小格子是否能向左移动
                // 6.4更新内容
                event.preventDefault();//移除默认按键下带来的效果(例如移除按左键,滚动条向左滑动)
...
// 6.2更新内容
//监听touchstart这个事件,当监听到这个事件时,响应function(event)里的函数,event存储着和touchstart相关的信息
...
// 6.4更新内容
//移除用户使用滑动动作时的默认事件(例如:向下滑动,进度条上移),该处会报错
// 错误内容:Unable to preventDefault inside passive event listener due to target being treated as passive
...
document.addEventListener('touchend',function(event){
    ...
    // 6.3更新
    var deltaX = endX - startX;//水平方向移动的距离
    var deltaY = endY - startY;//垂直方向移动的距离

    // 6.4更新内容
    // 有一点待完善的地方是:touchstart的触发点应该在主棋盘格里才更恰当,添加如下代码:
    ...
    //6.4更新内容
    //判断用户动作是滑动,还是点击
    //如果滑动的距离小于手机可显示尺寸的0.3倍时,判定为点击动作而不是滑动,此时不执行下列语句
    if(Math.abs(deltaX) < 0.3*documentWidth && Math.abs(deltaY) < 0.3*documentWidth){
        return;
    }

    if(Math.abs(deltaX) >= Math.abs(deltaY)){
        // 如果水平滑动距离大于垂直滑动距离,则代表该次滑动是水平滑动(左右滑)
        if(deltaX > 0){
            //向右滑动
            if(moveRight()){//判断是否能向右移动
                setTimeout("generateOneNumber()",300);
                setTimeout("isgameover()",330); 
            }
        }
        else{
            //向左滑动
